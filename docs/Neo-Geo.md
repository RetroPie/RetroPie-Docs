...
lr-fbalpha is the prefered Pi 2/Pi 3 Neo Geo emulator due to its accuracy. lr-fbalpha2012 is useful for any games that may be running slow in the latest version of lr-fbalpha (most notably the non-Neo Geo Konami 4-player beat'em up games) and, if used exclusively instead of lr-fbalpha, will allow you to do full system updates without worrying about needing to update your ROM Set, but comes at the cost of inaccuracy, with most notably Neo Geo games having minor audio bugs across the entire library. PiFBA is recommended for those on a Pi 0 or Pi 1.
