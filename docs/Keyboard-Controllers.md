...
Before long, some enterprising individuals realized that they could pop open their computer keyboard to gain access to the circuit board and solder wires to actual arcade parts (joystick switches and pushbuttons) which were available from manufacturers for servicing arcade systems. It was possible to build an arcade cabinet from scratch using real arcade controls and wire them to a computer keyboard and play emulated games.
